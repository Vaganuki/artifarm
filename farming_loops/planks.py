...
from data.locations import MINING_WORKSHOP, WOODCUTTING_WORKSHOP
...
async def plank_cycle(TOKEN ,CHARACTER_NAME , WOOD_CODE, PLANK_CODE, LOCATION):
    headers = {
    ...
    while True:
        ...
        if await find_other_items(char_data['inventory'], WOOD_CODE):
            await deposit_except_item(TOKEN, char_data, headers, WOOD_CODE)
        ...
        try:
            ...
            await move_to(TOKEN, character=char_data, x=WOODCUTTING_WORKSHOP['x'], y=WOODCUTTING_WORKSHOP['y'])
            plank_craft = await find_item(char_data["inventory"], WOOD_CODE)
            if plank_craft:
                try:
                    print(type(plank_craft['quantity']), plank_craft)
                    c_err = await craft_item(headers,crafting_url,PLANK_CODE, plank_craft['quantity'] // 10)
                    if c_err:
