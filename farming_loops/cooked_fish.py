...
from data.locations import COOKING_WORKSHOP
...
async def cooked_fish_cycle(TOKEN ,CHARACTER_NAME , FISH_CODE, COOKED_FISH_CODE, LOCATION):
    headers = {
    ...
    while True:
        ...
        if await find_other_items(char_data['inventory'], FISH_CODE):
            await deposit_except_item(TOKEN, char_data, headers, FISH_CODE)
        ...
        try:
            ...
            await move_to(TOKEN, character=char_data, x=COOKING_WORKSHOP['x'], y=COOKING_WORKSHOP['y'])
            fish_cook = await find_item(char_data["inventory"], FISH_CODE)
            if fish_cook:
                try:
                    c_err = await craft_item(headers,crafting_url,COOKED_FISH_CODE, fish_cook['quantity'])
                    if c_err:
